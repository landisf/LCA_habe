...
Findings from this strand of the literature can be used to make statements about responsibility-based obligations of different countries to contribute to the global effort of reducing \ac{ghg} emissions.\footnote{%
...
Upon a second look, an additional strand of the discussion around distributional issues can be identified in the academic literature.
It focuses on exposure of households to user-pays climate policy (i.e., climate policy that is not primarily relying on subsidizing clean technologies) and its conclusions can be relevant for informing policy design.
...
In this paper I shed light on both aspects of distributional analysis.
I do so by analysing the \ac{gwp} associated with the \ac{ghg} emissions embodied in the consumption of different categories of goods across households of Switzerland.
...
Comparing \ac{gwp} embodied in households' consumption patterns with their expenditure budgets, on the other hand, gives me a measure of their exposure to user-pays climate policy that addresses emissions from all world regions and all \acp{ghg}.\footnote{%
...
Second, the paper discusses exposure of households in different income classes to user-pays climate policy if climate policy extends in scope both spatial and in terms of \ac{ghg} coverage.
Even if the analysis lacks rigour and realistic policy detail compared to previous studies of \ac{eu} and Swiss climate policy \citep[e.g.,][]{landis_efficient_2019, landis_between-_2021} it provides an outlook and allows anticipating qualitative outcomes.
...
I note that a (costly) reduction of the emissions form the latter group of goods would not impact consumer price indices of the lowest-income groups much but would on its own also not come close to reaching a net-zero emissions target.
...
Second, emissions (and thus exposure to policies that increase consumer prices) rise much slower with income for heating than they do for transport.
...
For both categories, low-income households emit more \acp{ghg} per CHF of total household expenditure (and are thus likely to incur higher direct costs from reducing emissions relative to their overall consumption budget) than do high-income households.
...
In this section, I focus on what I call ``user-pays'' climate policies (they directly increase the cost of living for consumers)\footnote{%
  If the (future) benefits of a better climate are neglected, all climate policy is bound to be costly to \emph{society}:
  ...
  What I focus on here, is \emph{cost to consumers} in the sense, that the policies increase consumer prices.

  In my analysis, I assume that emission free technologies for producing different consumption goods are more expensive than their polluting counterparts at least in the short and medium run.
  (The `short run' is the time frame in which buying and operating clean technologies is more expensive or is (perceived as) disadvantageous compared to buying and operating \ac{ghg} emitting technologies.
...
This focus is motivated by the fact that it is mostly around user-pays climate policies that concerns about the distribution of policy impacts have been raised.
...
Some may say that is not fair and prefer mandates (or even subsidies) for regulating \ac{ghg} emissions, others may be content to know that at least non-reducing parties pay for their emissions under emissions pricing, thus contributing to public funds.
...
In the following, I explore some options for targeting specific consumption goods with climate policy that allow protecting low-income households from disproportionate policy impacts and make sure that high-income households do their fair share -- or pay up.
...
User-pays policies that target such consumption goods specifically are almost certainly costing low-income households very little.
...
The following section discusses to what extent low-income households may be impacted disproportionally by user-pays policies and makes some suggestions for policy designs to avoid this.
...
\citet{landis_efficient_2019}, among others, show that for policy design similar to the current Swiss policy framework, carbon pricing policies can be expected to keep societal costs of achieving emissions reduction significantly lower than policy packages that focus on mandating and subsidizing specific technologies alone.
...
\begin{figure}[htp]
  ...
  \caption[Welfare impacts of carbon pricing]{Welfare impacts of carbon price based climate policy for Switzerland in 2035 (panel A, left) and 2050 (panel B, right) according to \citet{landis_cost_2019}.
...
This rendering shows that, in terms of averages (depicted by circles in the figure), the net benefits relative to household spending are larger for low-income households than the simplified net costs (direct costs minus refunds) for high-income households.
...
Looking at \ac{gwp} per capita across household types I show that there are consumption categories where high income households spend a larger share of nationwide spending than for other goods. Among the more prominent among these are aviation, recreational activities, eating out in restaurants, and clothing. Aviation is a particularly meaningful category, since it represents a luxury good and its emissions are considerable, especially as long as no effective ways of mitigating non-\ac{co2} effects of aviation are in place.
To the extent that policy makers worry about the distribution of the emissions reduction effort and of cost of user-pays policy, they should implement policies that focus on these consumption categories.
...
Comparing emissions across other household categorizations reveals that (i) going for emissions reductions in transport takes more effort from the working age households since the elderly tend to travel less and it would affect peri-urban and rural communities  more than urban ones and (ii) going for emissions reductions in heating has higher impacts for the elderly and households who own their dwelling.

When I compare \ac{gwp} intensity across different household types, I come the conclusions that direct impacts of user-pays climate policy are skewed more towards progressivity if goods are targeted that display high consumption levels by high-income households.
...
What my study shows is that regressive \emph{distribution} of impacts of emissions pricing can be avoided and that arguments of protecting the poor should not be a major stumbling block to implementing emissions pricing for additional goods.
...
The scripts and additional data for reproducing the findings and figures in this study are available in the Git repository ``LCA\_HABE'', \url{https://github.com/landisf/LCA_habe}.
...
For heating, even after splitting up general utility expenditures, approximately one third of households does not have any expenditures for heating.
...
Transport and heating are the two most important components of household consumption in terms of \ac{ghg} emissions.
But when it comes to creating progressive outcomes (or on the flip-side creating incentives for high income households to act against climate change) more consumption categories merit being considered:
...
In all of these cases, the central 50 percent of the first income decile would achieve positive outcomes and it would be largely the high income households that would be negatively affected by such policies.
