...
Similarly, I find that home owners, on average, emit more per capita than do residents who rent their home and that residents of peri-urban areas (the ``Agglomeration'') have the the highest and the residents of urban areas the lowest per-capita \ac{gwp} among the three degrees of urbanization (even though this trend is not very distinctive).
...
The bottom panel of Figure~\ref{fig:gwp_cat_mob} shows that the categories of private transport make up a bigger share of total household consumption in lower income deciles than in in higher-income ones.
...
This technically works best if the luxury version of a good is also more emissions-intensive like \acp{suv} or muscle cars in the example of trasport.
...
While revenue recycling makes all policies decisesively progressive in terms of mean decile impacts, Figure~\ref{fig:multibox} shows the difference that \emph{extra} progressivity can make for the hardest hit in the lowest income decile.
...
When I compare \ac{gwp} intensity across different household types, I come the the conclusions that costly emissions reduction efforts are sqewed more towards progressivity if goods are targeted that display high consumption levels by high-income households.
...
The current practice of applying the Swiss \ac{co2} levy mainly on heating fuels results in less progressive outcomes than would a levy on transport fuels (or on a hypothetical levy on all \ac{ghg} emissions embodied in overall consumption).
...
While the \ac{eu} has decided to regulate additional parts of its economy with a carbon price when it created the so called \ac{ets}2 for heating and tranport fuels, it is far from certain that Switzerland will follow suit.
...
Analyses like mine have to be seen less as finalized policy suggestions rather than as inputs to evaluations of what may be politically desirable or feasible overall.
