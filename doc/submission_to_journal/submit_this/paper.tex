...
This makes low-income households more vulnerable to cost increases due to carbon pricing and requirements to adopt expensive technologies for emissions reductions, and such policies have to be expected to have regressive distributional effects.\footnote{%
...
It is the purpose of the next section to analyze and discuss these differences in consumption patterns in more depth.
...
This highlights the fact that we cannot get around increasing the cost of consumption for low-income households if Switzerland's net-zero emissions target is to be met (I assume that emission free technologies for producing different consumption goods are more expensive than their polluting counterparts at least in the short and medium run\footnote{%
  Let me specify the `short run' as the time frame in which buying and operating clean technologies is more expensive or is (perceived as) disadvantegeous compared to buying and operating \ac{ghg} emitting technologies.
...
} and that the state will not fully take over these additional costs).

In the following, I explore some options for targeting specific consumption goods with climate policy that allow protecting low-income households from disproportionate policy cost and make sure that high-income households do their fair share.
...
If emissions pricing is chosen as the policy instrument, revenue recycling through a per-capita lump-sum scheme can ensure progressive outcomes.
The current practice of applying the Swiss \ac{co2} levy mainly on heating fuels results in less progressive outcomes than would a levy on transport fuels (or a hypothetical levy on all \ac{ghg} emissions embodied in overall consumption).
...
Since the effects on consumers of such subsidies may correlate more with vintages and quality of devices and installations owned by households and are to a lesser extent proportional to current emissions, the \ac{gwp}  data analyzed here has less to say about subsidies.
...
Thus, the analysis does not take into account potential changes in consumption patterns or efficiency improvements since the survey was conducted let alone give a outlook of how consumption patterns may look in the future.
